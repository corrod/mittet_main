...
!      k=nz/2+1
      ...
      ez(i,j,k)=ez(i,j,k)-(dt/eps_p(id))*(1.0d0/(dx*dy))*func_gaussian1(time-dt/2.0d0)
...
real(8) function sin_warming_up(t)
  ...
  t1=3.0d0*tp
  if(t.lt.t1) then
     sin_warming_up=0.5d0*(1.0d0-dcos(pi*t/t1))*dsin(2.0d0*pi*(1.0d1*freq)*t)
  else
     sin_warming_up=dsin(2.0d0*pi*(1.0d1*freq)*t)
  end if
end function
