...
real(8) function func_gaussian(t)
  ...
  tp=1.0d0/freq
!  if(t.lt.tp*2.4) then
     ! 正規分布: exp(-(x-m)^2/(2 s^2))
     func_gaussian=dexp(-(t-tp/2.0d0)**2/(2.0d0*(tp/4.0d0)**2))
 ! else
 !    func_gaussian=0.0d0
 ! end if
end function
...
real(8) function func_gaussian1(t)
  ...
  func_gaussian1=-2.0d0*beta*(t-tp+150.0*dt)*dsqrt(beta/pi)*dexp(-beta*(t-tp+150.0d0*dt)**2)*1.0d-19
