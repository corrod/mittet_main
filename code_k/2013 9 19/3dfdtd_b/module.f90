...
module fdtd
  ...
  ! *** フィールド ***
  integer,parameter :: mx=120,my=120,mz=120
  integer :: nx,ny,nz
  real(8) :: dx,dy,dz
  ...
  real(8) :: cxd,cyd,czd,cxu,cyu,czu
  real(8) :: cxx,cyy,czz
  real(8) :: cxfyd,cxfzd,cyfxd,cyfzd,czfxd,czfyd
  ...
  ! *** PML吸収境界条件の係数 ***
  integer :: i0,i1,j0,j1,k0,k1,l1,l2,l3,lpmlst(6),m,lpml
  integer :: pmlxs,pmlxe,pmlys,pmlye,pmlzs,pmlze 
  real(8) :: lpmlii(6,2),lpmljj(6,2),lpmlkk(6,2),R0
  real(8) :: exy(mx,my,mz),exz(mx,my,mz),eyx(mx,my,mz),eyz(mx,my,mz),ezx(mx,my,mz),ezy(mx,my,mz)
  real(8) :: hxy(mx,my,mz),hxz(mx,my,mz),hyx(mx,my,mz),hyz(mx,my,mz),hzx(mx,my,mz),hzy(mx,my,mz)
  real(8) :: cxe(mx),cye(my),cze(mz)
  real(8) :: cxh(mx),cyh(my),czh(mz)
  real(8) :: cxel(mx),cyel(my),czel(mz)
  real(8) :: cxhl(mx),cyhl(my),czhl(mz)
  real(8) :: shx(mx),shy(my),shz(mz),sex(mx),sey(my),sez(mz)
  real(8) :: shmx,semx
