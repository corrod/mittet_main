...
# derivative of absolute residual
dy_stac = ( data3[:,0] - data3[:,1] ) / 2.
for i, case in enumerate(cases):
	y1 = data3[:,case-left+1]
	y2 = data3[:,case-left+2]
	dy = (y2-y1)/2.
	dy_stac = np.c_[dy_stac,dy]
np.savetxt('combine_residual_abso_derivation.dat',dy_stac)
...
contourf(X,Y,dy_stac)
colorbar()
savefig('combine_residual_abso_derivation.png')
show()


# derivative of differeantial residual
dy_stac = ( data4[:,0] - data4[:,1] ) / 2.
for i, case in enumerate(cases):
	y1 = data4[:,case-left+1]
	y2 = data4[:,case-left+2]
	dy = (y2-y1)/2.
	dy_stac = np.c_[dy_stac,dy]
np.savetxt('combine_residual_diff_derivation.dat',dy_stac)
...
contourf(X,Y,dy_stac)
colorbar()
savefig('combine_residual_diff_derivation.png')
show()