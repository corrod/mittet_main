...
module const_para
    ...
    integer :: i, j, k
    integer, parameter :: nstep = 3433 ! 総タイムステップ数 　
    integer, parameter :: nx=51, ny=51, nz=165  !グリッド数 奇数　
...
!model
    ...
    integer, parameter :: offset = 30        !送信源ープレート間の距離
    integer, parameter :: L_ver = 10         !パターン１(２)の受信間距離(縦)
    integer, parameter :: L_hori = 10        !パターン１(２)の受信間距離(横)
    integer, parameter :: L_sr = 5           !パターン２の送受信距離(縦)
    integer, parameter :: x0 = (nx+1)/2, y0 = (ny+1)/2, z0 = (nz+1)/2 !中心位置

    !パターン1　ソース位置＝レシーバ①
    integer, parameter :: x1 = x0,          y1 = y0, z1 = plate + offset  !レシーバ位置①
    integer, parameter :: x2 = x1,          y2 = y1, z2 = z1 - L_ver      !レシーバ位置②
    integer, parameter :: x3 = x1 + L_hori, y3 = y1, z3 = z1              !レシーバ位置③
    integer, parameter :: x4 = x1 + L_hori, y4 = y1, z4 = z1 - L_ver      !レシーバ位置④

    !パターン2　ソース位置＝レシーバ①②間
    integer, parameter :: x_source = x0, y_source = y0, z_source = plate + offset !送信源位置
    integer, parameter :: x1 = x0,          y1 = y0, z1 = z_source + L_sr   !レシーバ位置①
    integer, parameter :: x2 = x1,          y2 = y1, z2 = z1 - L_ver        !レシーバ位置②
    integer, parameter :: x3 = x1 + L_hori, y3 = y1, z3 = z1                !レシーバ位置③
    integer, parameter :: x4 = x1 + L_hori, y4 = y1, z4 = z1 - L_ver        !レシーバ位置④
