	...
	do k=2,nz-2
		do i=3,nx-2
			...
			ez(i,ny,k) = -ezy2(i,3,k)+cyd*(ez(i,ny-1,k)+ezy2(i,4,k))&
						...
						+cyfxd*(ezy1(i+1,4,k)-2.0d0*ezy1(i,4,k)+ezy1(i-1,i,k)&
						+ezy1(i,3,ki+1)-2.0d0*ezy1(i,3,k)+ezy1(i,3,k-1))&
						+cyfzd*(ezy1(i,4,k+1)-2.0d0*ezy1(i,4,k)+ezy1(i,4,k-1)&
						+ezy1(i,3,k+1)-2.0d0*ezy1(i,3,k)+ezy1(,3,k-1))
		enddo
	enddo
	!過去の値の更新
	do k=1,nz-1
...
!---------eyに対して-----------------------------------------------------------------
	...
	do i=3,nx-2
		...
		ey(i,j,nz) =  eyz1(i,j,3)+czu*(ey(i,j,ny-1)-eyz1(i,j,4))
		j=ny-1
		ey(i,j,1) = eyz1(i,j,2)+czd*(ey(i,j,2)-eyz1(i,j,1))
		ey(i,j,nz)= eyz1(i,j,3)+czu*(ey(i,j,ny-1)-eyz1(i,j,4))
	enddo
