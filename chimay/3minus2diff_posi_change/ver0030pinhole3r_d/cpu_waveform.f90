...
!                     - signal(istep)*dt / myu(x_source2, y_source2, z_source2) /dx/dy/dz!　　　
 ...
 subroutine read_source_3d_res(istep,t,Hz)
    ...
    !ソース波形
    write(14,*) t_res(istep), signal_res(istep)
...
!ソース　センター
    Hz(sp, y_source, z_source - L_sr) = Hz(sp, y_source, z_source - L_sr) &
                    - signal_res(istep) * dt / myu(sp, y_source, z_source - L_sr) /dx/dy/dz!
